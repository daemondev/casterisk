...
int main(int argc,char *argv[])
{
  ...
  int sock;
  int choice;
  char buf[MAXDATASIZE], command[5], filename[20], *f;
  int k, size, status;
  int filehandle;
#ifdef WIN32
    WSADATA wsaData;
  ...
  while(1)
	...
	case 2:
	  ...
	  /*sendfile(sock, filehandle, NULL, size);*/
          TransmitFile(sock, &filehandle, size, size, NULL, NULL, MAXDATASIZE);
	  recv(sock, buf, MAXDATASIZE, 0);
	  if(status)
